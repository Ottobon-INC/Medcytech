...
export function useThreeScene(
  ...
  useEffect(() => {
    ...
    const updateSize = () => {
      ...
      if (window.innerWidth < 1024) targetWidth = 9.5; // Tighter bounds on tablet
      if (window.innerWidth < 600) targetWidth = 7.5;  // Even tighter bounds on mobile so it stays large
